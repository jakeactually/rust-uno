...
impl Card {
    ...
    pub fn matches(
        ...
        match game_state {
            ...
            Some(GameState::Plus4) => {
                make_error(other == Card::Plus4, "You can only chain or pass")
            }
    ...
    pub fn free_match(self, choosen_color: CardColor, other: Card) -> Option<String> {
        match self {
            ...
            Card::Stop(color) => {
                make_error(other.is_stop() || other.matches_color(color), "Wrong color")
            }
            Card::Reverse(color) => make_error(
                other.is_reverse() || other.matches_color(color),
                "Wrong color",
            ),
            Card::Plus2(color) => make_error(
                other.is_plus_2() || other.matches_color(color),
                "Wrong color",
            ),
            Card::ChangeColor => make_error(
                other.matches_color(choosen_color.clone()),
                format!("Choosen color is {}", choosen_color).as_str(),
            ),
            Card::Plus4 => make_error(
                other.matches_color(choosen_color.clone()),
                format!("Choosen color is {}", choosen_color).as_str(),
            ),
        }
...
pub fn all() -> Vec<Card> {
    ...
    vec![
        colors
            .iter()
            .map(|color| Card::Number(0, color.clone()))
            .collect(),
        (1..9)
            .flat_map(|n| colors.iter().map(move |c| Card::Number(n, c.clone())))
            .collect(),
        colors
            .iter()
            .map(|color| Card::Stop(color.clone()))
            .collect(),
        colors
            .iter()
            .map(|color| Card::Reverse(color.clone()))
            .collect(),
        colors
            .iter()
            .map(|color| Card::Plus2(color.clone()))
            .collect(),
