...
use crate::user::User;
use crate::room::Room;
use crate::message::{UnoMessage};
use crate::uno::{Uno, room_and_player};
use serde::{Serialize, Deserialize};
use rand::seq::SliceRandom;
use rand::thread_rng;
use crate::card::{CardColor, Card, GameState};
...
pub async fn play(data: web::Data<Mutex<Uno>>, req: HttpRequest, session: Session) -> impl Responder {
    ...
    if !room.active {        
        room.deck = all.iter().enumerate().map(|(i, c)| (i as u8, c.clone())).collect();
...
pub async fn turn(data: web::Data<Mutex<Uno>>, req: HttpRequest, session: Session, form: web::Json<TurnReq>) -> impl Responder {
    let mut context = data.lock().unwrap();
    let all = context.cards.clone();
    let (room_id, player_id) = room_and_player(req, session);
    let room = context.rooms.get_mut(&room_id).unwrap();
    ...
    match card_1.1.matches(room.state.clone(), choosen_color, card_2.clone()) {
...
async fn notify(context: &mut MutexGuard<'_, Uno>, room_id: u32) {
    ...
    for subscription in subscribers.iter() {
        let _ = subscription.send(UnoMessage).await;
    }
}
...
pub fn do_turn(room: &mut Room, player_id: u32, card_tuple: (u8, Card)) {
    ...
    player.hand = player.hand.clone().into_iter().filter(|(id, _)| *id != card_tuple.0).collect();
    player.drawed = false;
    room.board.push(card_tuple);
    room.next();
}

pub async fn draw(data: web::Data<Mutex<Uno>>, req: HttpRequest, session: Session) -> impl Responder {
    let mut context = data.lock().unwrap();
    let (room_id, player_id) = room_and_player(req, session);
    let room = context.rooms.get_mut(&room_id).unwrap();
    let card = room.draw();
...
pub async fn penalty(data: web::Data<Mutex<Uno>>, req: HttpRequest, session: Session) -> impl Responder {
    let mut context = data.lock().unwrap();
    let (room_id, player_id) = room_and_player(req, session);
    let room = context.rooms.get_mut(&room_id).unwrap();
...
pub async fn pass(data: web::Data<Mutex<Uno>>, req: HttpRequest, session: Session) -> impl Responder {
    let mut context = data.lock().unwrap();
    let (room_id, player_id) = room_and_player(req, session);
    let room = context.rooms.get_mut(&room_id).unwrap();
    let player = room.player(player_id);
